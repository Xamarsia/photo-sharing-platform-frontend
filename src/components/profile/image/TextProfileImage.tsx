...
import styles from '@/styles/components/profile.image.module.css';
...
const textColours: Array<string> = ['text-gray-400, text-red-400', 'text-orange-400', 'text-amber-400', 'text-yellow-400', 'text-lime-400', 'text-green-400', 'text-emerald-400', 'text-teal-400',
...
const borderColours: Array<string> = ['border-gray-200, border-red-200', 'border-orange-200', 'border-amber-200', 'border-yellow-200', 'border-lime-200', 'border-green-200', 'border-emerald-200', 'border-teal-200',
...
export default function TextProfileImage({ username, preview }: Props) { //DefaultProfileImage
    const index = useMemo(() => getColourIndex(username), [username]); //getColorIndex
